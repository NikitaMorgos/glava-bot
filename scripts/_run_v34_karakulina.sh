#!/usr/bin/env bash
# Karakulina v34: Stage1(TR1+TR2)->Stage2(+historian)->Stage3 — Phase A только
# Ghostwriter v2.13 (универсальный), FC v2.6, temp=0.4
...
if "v2.13" not in gw["prompt_file"]:
    errors.append(f"  ❌ Ghostwriter prompt={gw['prompt_file']} (требуется v2.13)")
if "v2.6" not in fc["prompt_file"]:
    errors.append(f"  ❌ Fact Checker prompt={fc['prompt_file']} (требуется v2.6)")
...
if errors:
    ...
    print()
    print("  ✅ Конфигурация v34 подтверждена (GW v2.13 temp=0.4, FC v2.6, LD v3.19)")
PREFLIGHT
...
echo ""
echo ">>> STAGE 2: Historian v3 + Ghostwriter v2.13 + FC v2.6 (historian ENABLED)"
...
cp "$FACT_MAP"  "$COLLAB_RUN/fact_map_v34.json"
[ -n "$FACT_MAP_FULL" ] && cp "$FACT_MAP_FULL" "$COLLAB_RUN/fact_map_full_v34.json"
cp "$BOOK_S2"   "$COLLAB_RUN/book_FINAL_stage2.json"
cp "$BOOK_S3"   "$COLLAB_RUN/book_FINAL_stage3_v34.json"
test -n "$TXT_S3" && cp "$TXT_S3" "$COLLAB_RUN/karakulina_v34_FINAL_phase_a.txt"
...
echo " Gate 1: проверь karakulina_v34_FINAL_phase_a.txt"
echo "=============================================="
