#!/usr/bin/env bash
# Karakulina v30: Stage1->Stage2(+historian)->Stage3->PhaseB
# Ghostwriter v2.10, FC v2.4, Istorik v3 ENABLED
set -euo pipefail
...
echo " Tag:  $PREFIX"
echo " Time: $(date)"
echo " Pisatel: v2.10 (zapret simvolizacii, raznesenie spravok)"
echo " FC: v2.4 (framing_distortion patern)"
...
echo ""
echo ">>> STAGE 2: Istorik v3 + Ghostwriter v2.10 + FC v2.4 (historian ENABLED)"
...
cp "$FACT_MAP" "$COLLAB_RUN/fact_map_v30.json"
cp "$BOOK_S2"  "$COLLAB_RUN/book_FINAL_stage2.json"
cp "$BOOK_S3"  "$COLLAB_RUN/book_FINAL_stage3.json"
cp "$BOOK_PB"  "$COLLAB_RUN/book_FINAL_phase_b_v30.json"
...
printf "# Run: %s (%s)\n\nGhostwriter v2.10, FC v2.4, Istorik ENABLED\nTR1: assemblyai\nTR2: meeting transcript\n\n## Izmeneniya v30\n- Zapret simvolizacii koloritnyx slov\n- Pravilo razneseniya spravok (min 1 abzac mezhdu nimi)\n- framing_distortion v FC v2.4\n- CondPageBreak 28mm vmesto 50mm\n" "$PREFIX" "$TS" > "$COLLAB_RUN/README.md"

echo "${COLLAB_RUN}" > "$EXPORTS/karakulina_v30_last_collab_run.txt"
