...
def _extract_prompt_version(prompt_file: str) -> str:
    ...
    return m.group(1) if m else (prompt_file or "unknown")
...
def main():
    ...
    save_run_manifest(
        ...
        notes={
            "strict_gates_enabled": not args.no_strict_gates,
            "variant_b": args.variant_b,
            "ghostwriter_version": _extract_prompt_version(
                cfg.get("ghostwriter", {}).get("prompt_file", "")
            ),
            "completeness_auditor_version": _extract_prompt_version(
                cfg.get("completeness_auditor", {}).get("prompt_file", "")
            ),
        },
    )
