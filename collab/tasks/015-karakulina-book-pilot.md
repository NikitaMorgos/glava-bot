...
**Текущий этап:** надёжность пайплайна — в работе. 8 архитектурных задач закрыты в `done` (016/017/018/019/020/021/022/023). 3 задачи в `spec-approved`, ожидают Cursor (024/025/026).
...
- ⏳ Визуальная корректность (chapter_start, markdown headings) — 024, 025 готовы к реализации
- ⏳ Содержательная стабильность (Stage 1) — 026 готов к реализации

**Не блокеры — просто ещё не сделано:**
- gate3 (фото) и gate4 (обложка) — НЕ запускаются на Каракулиной до закрытия 024/025/026
- Это сознательное решение: gate3/4 будут подтверждением системы, не самоцель
