...
### 2026-04-28 — Claude: финальная приёмка v36, задача `done`

Все 6 критериев приёмки выполнены:

| Критерий | Результат |
|---|---|
| Татьяна в `persons[]` | ✅ v36a — через CA auto_enrich; v36b — FE нашёл сам |
| 5 ключевых фактов TR2 | ✅ все в CA auto_enrich (Сахалин, пианино/шуба, сервиз, «дорожка», Нинвана) |
| Маргось Владимир / Владимир Маргось слиты | ✅ aliases=['Владимир Маргось'] |
| Manifest содержит `completeness_audit` + `name_normalization` | ✅ |
| `validate_fact_map_integrity()` | ✅ 0 ошибок |
| `stability_score ≥ 90%` | ✅ **95% (18/19 совпали)** |

**Артефакты:** `collab/runs/karakulina_v36_20260428/`

**Бонус-фиксы Cursor сверх ISSUE-1/2/3** (ценные наблюдения для будущих задач):

1. **CA max_tokens 8000 → 32000** — Haiku truncate'ил вывод на 41K-символьном транскрипте даже при 16000. Уроки на будущее: для агентов на длинном контексте сразу ставить max_tokens близко к лимиту модели и проверять `truncated`-флаг.
2. **STOP_TOKENS в Name Normalizer** — role-слова («тётя», «дядя», «дед», «село») не должны участвовать в матчинге, иначе ложные слияния.
3. **min 2 shared name tokens** — предотвращает слияние Каракулин Дмитрий (муж) и Каракулин Валерий (сын) через одну фамилию. Критичная защита, без неё Name Normalizer схлопывал бы родственников с одной фамилией.
4. **Транзитивность ID-remap** — корректная обработка цепочек A→B→C при множественных слияниях.
5. **Locations: no-aliases matching** — для топонимов используем только canonical name (asr_variants как контекст-ассоциации создавали ложные слияния).

**Аномалия `Сафронов` (−1 между прогонами):** в v36a CA добавил как low-confidence без чёткого biographic role; в v36b ни FE ни CA не зафиксировали. Скорее всего шум транскрипта (упомянут вскользь без связи с героем). На stability_score не повлияло (95%) — приёмка пройдена.

**Что задача дала проекту:**
- Архитектурная защита от нестабильности FE (теперь LLM может пропустить — Auditor подберёт)
- Детерминированная нормализация имён (одинаковые персоны больше не дублируются)
- Базовая защита от исторических фигур в коде (две линии: промпт Auditor + код Normalizer)
- Регрессионный инструмент `compare_persons_across_runs.py` для будущих прогонов
- Новый паттерн: «LLM-extractor → LLM-auditor → deterministic-normalizer» — применим к другим этапам пайплайна (например, locations, events)

**Что осталось как технический долг (не блокирует, но стоит держать в виду):**
- `_pick_canonical()` в Name Normalizer имеет неоднозначную логику когда длина имени и confidence противоречат — на текущих данных ОК, но на edge-cases может выбрать «не ту» каноническую форму. В будущем — добавить unit-test'ы.
- Словарь диминутивов (DIMINUTIVES) — захардкожен в коде. Если появятся проекты с редкими именами, нужен механизм проектного override.
- pymorphy2 не используем — для редких склонений (ASR-варианты с орфографией) могут быть пропуски слияний. Подключим если возникнет конкретный кейс.

Задача переходит в `done`. Возвращаемся к более крупным темам — task 013 (CJM production errors) или 015 (web order flow).
