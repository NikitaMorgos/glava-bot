"""
Пайплайн: голосовое -> AssemblyAI транскрипция -> LLM биография -> сохранение в папку пользователя.
"""
...
def run_pipeline_sync(
    ...
    """
    assemblyai_key = getattr(config, "ASSEMBLYAI_API_KEY", None) or os.getenv("ASSEMBLYAI_API_KEY", "")
    openai_key = getattr(config, "OPENAI_API_KEY", None) or os.getenv("OPENAI_API_KEY", "")

    if not assemblyai_key:
        logger.warning("ASSEMBLYAI_API_KEY не задан, пропускаем AssemblyAI")
        return False
    ...
    try:
        ...
        transcript = transcribe_via_assemblyai(
            tmp_path, api_key=assemblyai_key, language_code="ru", speaker_labels=True
        )

        if not transcript or not transcript.strip():
            logger.warning("AssemblyAI вернул пустой транскрипт")
            return False

        # Сохраняем сырой транскрипт (assemblyai в заголовке для сравнения)
        ...
        new_block = f"\n\n--- Голосовое (voice_id={voice_id}) assemblyai {datetime.now().strftime('%Y-%m-%d %H:%M')} ---\n{transcript}"
        ...
        logger.info("Пайплайн (assemblyai) завершён: %s", client_dir)
        return True

    except Exception as e:
        logger.exception("Пайплайн assemblyai ошибка: %s", e)
        return False
    finally:
...
def run_pipeline_background(
    ...
    def _run():
        try:
            ...
            logger.exception("Фоновый пайплайн assemblyai: %s", e)

    t = threading.Thread(target=_run, daemon=True)
    t.start()
    logger.info("Пайплайн (assemblyai) запущен в фоне для voice_id=%s", voice_id)
