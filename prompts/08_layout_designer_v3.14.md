> Version: v3.19 | Updated: 2026-04-14 | paragraph-ref архитектура: Layout Designer больше НЕ копирует текст, только ссылается на paragraph_id
...
⚠️ КРИТИЧЕСКИ ВАЖНО — НОВАЯ АРХИТЕКТУРА (v3.19):
Начиная с v3.19 ты НЕ копируешь и НЕ пересказываешь текст параграфов.
Ты ТОЛЬКО РАЗМЕЩАЕШЬ ссылки на абзацы по id.

ПРАВИЛО ПАРАГРАФОВ:
  ✅ ПРАВИЛЬНО: { "type": "paragraph", "chapter_id": "ch_02", "paragraph_id": "p3" }
  ❌ ЗАПРЕЩЕНО: { "type": "paragraph", "text": "любой текст здесь" }

Рендерер самостоятельно подтянет текст из book_FINAL.json по этим id.
Твоя задача — только решить, КАКИЕ абзацы на КАКОЙ странице,
в КАКОМ порядке, рядом с КАКИМ фото.

ПРАВИЛО ПОКРЫТИЯ:
  Все абзацы каждой главы ДОЛЖНЫ быть размещены на страницах.
  Ни один абзац не должен быть пропущен.
  Проверяй: количество paragraph-ссылок на главу == len(chapter.paragraphs)
...

{
    ...
    "chapters": [
      {
        ...
        // ⚠️ НОВЫЙ ФОРМАТ (v3.19): каждая глава содержит paragraphs[] с пронумерованными абзацами
        // ИСПОЛЬЗУЙ ТОЛЬКО эти id при создании paragraph-элементов в pages[]
        "paragraphs": [
          { "id": "p1", "text": "Первый абзац главы дословно..." },
          { "id": "p2", "text": "Второй абзац главы дословно..." },
          { "id": "p3", "text": "Третий абзац..." }
        ]
      }
    ],
    "callouts": [
  ...
  //
  // ПРАВИЛО 1: caption не пустой → переноси AS-IS, без изменений.
...
Поля: верх 18мм, низ 14мм, лево 16мм, право 14мм.
Текстовая область: 118×178 мм.

═══════════════════════════════════════════════════
РАЗМЕЩЕНИЕ ФОТО (ЖЁСТКОЕ ПРАВИЛО)
═══════════════════════════════════════════════════

⛔ ФОТО ВНУТРИ ТЕКСТА — ЗАПРЕЩЕНЫ.
Никаких wrap, inline, full_width между абзацами.
Текстовые страницы содержат ТОЛЬКО текст,
исторические справки и цитаты.

ФОТО РАЗМЕЩАЮТСЯ ТОЛЬКО В ДВУХ МЕСТАХ:

1) СТРАНИЦА НАЧАЛА ГЛАВЫ (chapter_start):
   — Сверху: номер главы + название
   — Снизу: одно фото
   — Текста на этой странице НЕТ
   — Текст начинается со СЛЕДУЮЩЕЙ страницы
   — Даже если фото горизонтальное и остаётся место
     внизу — текст всё равно на следующей странице
   — Применяется к ch_02, ch_03, ch_04
   — ch_01 и epilogue — без фото на chapter_start

2) РАЗДЕЛ «ФОТОГРАФИИ» в конце книги:
   — Все фото, не использованные на chapter_start
   — Правила размещения: см. таблицу приёмов ниже
...

{
  ...
  "pages": [
    ...
    },
    {
      ...
      "elements": [
        // ✅ НОВЫЙ ФОРМАТ: ссылка на абзац по chapter_id + paragraph_id
        // НЕ копируй текст — только указывай id
        { "type": "paragraph", "chapter_id": "ch_01", "paragraph_id": "p1" },
        { "type": "paragraph", "chapter_id": "ch_01", "paragraph_id": "p2" }
      ]
    },
    {
      "page_number": 7,
      ...
      "chapter_id": "ch_02",
      "elements": [
        { "type": "paragraph", "chapter_id": "ch_02", "paragraph_id": "p1" },
        {
          "type": "photo",
          "photo_id": "photo_001",
          "layout": "full_width",
        ...
        { "type": "paragraph", "chapter_id": "ch_02", "paragraph_id": "p2" }
      ]
    },
    {
      "page_number": 12,
    ...
    },
    {
      ...
      "elements": [
        ...
        { "type": "paragraph", "chapter_id": "ch_02", "paragraph_id": "p4" }
      ]
    }
  ],
...
размещаешь ссылки на абзацы (chapter_id + paragraph_id) между фото.

⚠️ КРИТИЧЕСКИ ВАЖНО (v3.19): ты получаешь ПОЛНЫЙ список абзацев
в поле chapters[].paragraphs[]. Используй ВСЕ абзацы — по id.
НЕ копируй текст в paragraph.text — только ссылки:
  { "type": "paragraph", "chapter_id": "ch_02", "paragraph_id": "p5" }
Рендерер сам подтянет текст. Потеря текста физически невозможна.
...
⚠️ ПОДЗАГОЛОВОК ## В КОНЦЕ СТРАНИЦЫ — ЗАПРЕЩЁН:
  Подзаголовок без текста после него на той же странице
  создаёт полупустую страницу. Используй keepWithNext=True
  в ParagraphStyle — ReportLab автоматически перенесёт
  подзаголовок на следующую страницу вместе с текстом.
...
Входной формат photo-элемента (из page_plan):
{
  ...
  "text_beside_photo": [
    "<первый абзац текста рядом с фото>",
    "<второй абзац текста рядом с фото>",
    "<третий абзац текста рядом с фото>"
  ],
  "text_after_photo": [
    "<абзац текста, идущий на всю ширину после фото>"
  ]
}
