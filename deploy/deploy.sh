...
# 6. systemd (бот)
echo "[6/7] Установка systemd-сервиса бота..."
...
# 7. Личный кабинет
echo "[7/7] Установка личного кабинета..."
sudo cp "$APP_DIR/deploy/glava-cabinet.service" /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable glava-cabinet
sudo systemctl start glava-cabinet 2>/dev/null || true

echo ""
echo "=== Готово ==="
echo "Бот: systemctl status glava"
echo "Кабинет: systemctl status glava-cabinet (слушает 127.0.0.1:5000)"
echo "Логи: journalctl -u glava -f"
echo ""
echo "Кабинет: добавь CABINET_SECRET_KEY и TRUST_PROXY=1 в .env"
echo "Nginx + SSL: см. CABINET_DEPLOY.md"
