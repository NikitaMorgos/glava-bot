> Version: v3.22 | Updated: 2026-05-06 | ссылочная архитектура расширена на callouts (callout_ref) и historical_notes (historical_note_ref) — симметрично paragraph_ref/subheading_ref (волна 1.1, шаг 4). База: v3.21 (subheading), v3.20 (paragraph_ref).
...
⛔ КРИТИЧЕСКОЕ ПРАВИЛО: ССЫЛОЧНАЯ АРХИТЕКТУРА ВЫНОСОК (v3.22)
— Для type="callout" ЗАПРЕЩЕНО поле "text" с текстом
— ОБЯЗАТЕЛЬНО указывать "callout_ref" + "chapter_id"
— "callout_ref" = id выноски из book_FINAL.callouts[].id
  (значения callout_01, callout_02, ...)
— "chapter_id" = book_FINAL.callouts[].chapter_id (та же глава, что у выноски)
— ПРИМЕР ПРАВИЛЬНО:   {"type": "callout", "chapter_id": "ch_02", "callout_ref": "callout_01"}
— ПРИМЕР НЕПРАВИЛЬНО: {"type": "callout", "text": "Цитата из главы..."}
— Каждый callout_ref из book_FINAL.callouts[] должен встречаться в layout
  РОВНО ОДИН раз. Пропуск, дубль, перестановка между главами =
  ошибка валидации (validate_layout_fidelity).

⛔ КРИТИЧЕСКОЕ ПРАВИЛО: ССЫЛОЧНАЯ АРХИТЕКТУРА ИСТОРИЧЕСКИХ СПРАВОК (v3.22)
— Для type="historical_note" ЗАПРЕЩЕНО поле "text" с текстом
— ОБЯЗАТЕЛЬНО указывать "historical_note_ref" + "chapter_id"
— "historical_note_ref" = id справки из book_FINAL.historical_notes[].id
  (значения hist_01, hist_02, ...)
— "chapter_id" = book_FINAL.historical_notes[].chapter_id
— ПРИМЕР ПРАВИЛЬНО:   {"type": "historical_note", "chapter_id": "ch_02", "historical_note_ref": "hist_01"}
— ПРИМЕР НЕПРАВИЛЬНО: {"type": "historical_note", "text": "В 1933 году..."}
— ВАЖНО: каждый historical_note из book_FINAL.historical_notes[] ОБЯЗАН
  быть размещён в layout как элемент в соответствующей главе. Пропуск
  не допускается — справки писались Историком и должны попасть в книгу.
— Каждый historical_note_ref из book_FINAL.historical_notes[] встречается
  в layout РОВНО ОДИН раз. Дубль, перестановка между главами = ошибка
  валидации (validate_layout_fidelity).

ИНВАРИАНТ ПОЛНОТЫ (для всех 4 ref-типов):
paragraphs, subheadings, callouts, historical_notes из book_FINAL —
все должны попасть в layout, каждый ровно один раз, в правильной главе,
в порядке появления в book_FINAL. Текст рендерится не из layout, а из
book_FINAL по ref'у.
